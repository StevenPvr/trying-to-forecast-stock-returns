...
def add_state_features(data: pd.DataFrame, context: PriceContext) -> None:
    ...
    data["deep_state_range_expansion_rate_21d"] = (
        context.true_range_pct.gt(context.true_range_median_63d).astype(float).rolling(21).mean()
    )
    data["deep_state_gap_fill_rate_21d"] = gap_fill.astype(float).rolling(21).mean()
    data["deep_state_breakout_up_rate_63d"] = breakout_up.astype(float).rolling(63).mean()
    data["deep_state_breakout_failure_up_rate_63d"] = breakout_failure_up.astype(float).rolling(63).mean()
    ...
    data["deep_state_overnight_dominance_21d"] = overnight_dominance.rolling(21).mean()
    data["deep_state_intraday_dominance_21d"] = intraday_dominance.rolling(21).mean()
...
def _conditional_volume_ratio(context: PriceContext, mask: pd.Series) -> pd.Series:
    conditional_volume = as_series(context.volume.where(mask).rolling(21).mean(), context.index)
