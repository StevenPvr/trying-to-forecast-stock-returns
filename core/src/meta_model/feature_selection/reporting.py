...
"""Selection reporting and filtered-dataset validation."""
...
def validate_filtered_dataset_matches_selection(
    ...
    actual_feature_names = [
        ...
        and (
            pd.api.types.is_numeric_dtype(filtered_dataset[column_name])
            or is_structural_categorical_feature_column(column_name)
        )
    ]
    if sorted(actual_feature_names) != sorted(selected_feature_names):
